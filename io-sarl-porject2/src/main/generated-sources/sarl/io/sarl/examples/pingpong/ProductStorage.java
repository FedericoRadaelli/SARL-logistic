...
public class ProductStorage extends Agent {
  ...
  private void $behaviorUnit$Initialize$0(final Initialize occurrence) {
    ...
    _$CAPACITY_USE$IO_SARL_API_CORE_LOGGING$CALLER.info("Agente ProductStorage Inizializato.");
    this.matrix = MatrixManager.getMatrix();
    this.matrix.printMatrix();
  }

  private void $behaviorUnit$BoughtProduct$1(final BoughtProduct occurrence) {
    int Ptype = occurrence.raw;
    int Pnumber = occurrence.qty;
    int _readCell = this.matrix.readCell((Ptype - 1), 1);
    this.matrix.writeCell((Ptype - 1), 1, (_readCell - Pnumber));
    this.matrix.printMatrix();
    Logging _$CAPACITY_USE$IO_SARL_API_CORE_LOGGING$CALLER = this.$CAPACITY_USE$IO_SARL_API_CORE_LOGGING$CALLER();
    _$CAPACITY_USE$IO_SARL_API_CORE_LOGGING$CALLER.info("Database aggiornato!");
  ...
  private void $behaviorUnit$Destroy$2(final Destroy occurrence) {
    Logging _$CAPACITY_USE$IO_SARL_API_CORE_LOGGING$CALLER = this.$CAPACITY_USE$IO_SARL_API_CORE_LOGGING$CALLER();
    _$CAPACITY_USE$IO_SARL_API_CORE_LOGGING$CALLER.info("Agente ProductStorage terminato.");
  }
  ...
  @SyntheticMember
  @PerceptGuardEvaluator
  private void $guardEvaluator$BoughtProduct(final BoughtProduct occurrence, final Collection<Runnable> ___SARLlocal_runnableCollection) {
    assert occurrence != null;
    assert ___SARLlocal_runnableCollection != null;
    ___SARLlocal_runnableCollection.add(() -> $behaviorUnit$BoughtProduct$1(occurrence));
  }
  ...
  public void $getSupportedEvents(final Set<Class<? extends Event>> toBeFilled) {
    ...
    toBeFilled.add(BoughtProduct.class);
  }
  ...
  public boolean $isSupportedEvent(final Class<? extends Event> event) {
    ...
    if (BoughtProduct.class.isAssignableFrom(event)) {
      return true;
    }
    return false;
  }
  ...
  public void $evaluateBehaviorGuards(final Object event, final Collection<Runnable> callbacks) {
    ...
    if (event instanceof BoughtProduct) {
      final BoughtProduct occurrence = (BoughtProduct) event;
      $guardEvaluator$BoughtProduct(occurrence, callbacks);
    }
  }
  ...
  @SyntheticMember
  public ProductStorage(final UUID parentID, final UUID agentID) {
    super(parentID, agentID);
  }

  @SyntheticMember
  @Inject
  public ProductStorage(final UUID parentID, final UUID agentID, final DynamicSkillProvider skillProvider) {
